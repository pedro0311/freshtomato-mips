...
//	<% nvram("tor_enable,tor_solve_only,tor_socksport,tor_transport,tor_dnsport,tor_datadir,tor_users,tor_ports,tor_ports_custom,tor_custom,tor_iface,tor_ctrlport,lan_ifname"); %>
...
function verifyFields(focused, quiet) {
	...
	if (!v_port(E('_tor_ctrlport'), quiet || !ok)) ok = 0;
	if (!v_nodelim(E('_tor_datadir'), quiet || !ok, 'Directory', 1) || !v_path(E('_tor_datadir'), quiet || !ok, 1)) ok = 0;

	var s = E('_tor_custom');

	/*
	 * These options are generated by FreshTomato for the New Identity
	 * controller. Letting Custom Configuration redefine them could make the
	 * GUI action authenticate against a different or unprotected listener.
	 */
	if (/^\s*(ControlPort|ControlSocket|CookieAuthentication|CookieAuthFile|HashedControlPassword)\b/im.test(s.value)) {
		ferror.set(s, 'Tor control options are managed by FreshTomato and are not allowed in Custom Configuration.', quiet || !ok);
		ok = 0;
	}
...
var torNewnymXob = null;

function newIdentity() {
	var b = E('_tor_interface');

	if (!isup.tor || torNewnymXob)
		return;

	b.disabled = 1;

	if ((torNewnymXob = new XmlHttp()) == null) {
		form.submitHidden('service.cgi', { _service: 'tor_newnym-start', _sleep: 1, _redirect: 'advanced-tor.asp' });
		return;
	}

	torNewnymXob.onCompleted = function(text, xml) {
		torNewnymXob = null;
		show();
	};

	torNewnymXob.onError = function(ex) {
		torNewnymXob = null;
		show();
	};

	torNewnymXob.post('service.cgi', '_service=tor_newnym-start&_sleep=1&_ajax=1');
}
...
<div class="section">
	<div class="fields">
		<span id="_tor_notice"></span><input type="button" id="_tor_button">
		<input type="button" id="_tor_interface" value="New Identity" onclick="newIdentity();">
...
<div class="section" id="config-section">
	<script>
		createFieldTable('', [
			...
			{ title: 'Control Port', name: 'tor_ctrlport', type: 'text', maxlen: 5, size: 7, value: fixPort(nvram.tor_ctrlport, 9051) },
...
<div class="section">
	<ul>
		...
		<li><b>New Identity</b> - Requests clean circuits for new connections and clears Tor's client-side DNS cache. Existing connections are not moved. Tor may rate-limit this request.</li>
		<li><b>Control Port</b> - Localhost-only Tor control interface used by the New Identity action.</li>
