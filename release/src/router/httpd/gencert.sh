...
openssl req -new -sha256 -out /tmp/cert.csr -config $OPENSSLCNF -keyout /tmp/privkey.pem.$PID -newkey rsa:2048 -passout pass:password
# remove the passphrase from the key
openssl rsa -in /tmp/privkey.pem.$PID -out $KEYNAME.$PID -passin pass:password
...
rm -f /tmp/cert.csr /tmp/privkey.pem.$PID $OPENSSLCNF /var/run/gencert.pid
