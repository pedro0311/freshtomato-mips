...
/*
 * IPv4 sources that belong to the local side of a conntrack entry.
 *
 * LAN entries are networks because clients behind every configured bridge are
 * local endpoints. WAN entries are exact router-owned addresses; matching a
 * complete WAN subnet would also match unrelated hosts on the upstream
 * segment.
 *
 * get_wanfaces() can expose up to ASIZE(wanface_list_t.iface) addresses per
 * WAN unit (for example, a PPP tunnel address and its underlying WAN address).
 */
#define CTNF_WAN_ADDR_MAX	(MWAN_MAX * ASIZE(((wanface_list_t *)0)->iface))

struct ctnf_ipv4_local {
	struct {
		in_addr_t network;
		in_addr_t netmask;
	} lan[BRIDGE_COUNT];
	in_addr_t wan[CTNF_WAN_ADDR_MAX];
	unsigned int lan_count;
	unsigned int wan_count;
};

/*
 * Build the IPv4 source set once for an ASP request.
 *
 * Bridge-scoped NVRAM access uses FreshTomato's shared lan/lanN helpers.
 * WAN enumeration is limited to the configured MultiWAN count, and
 * get_wanfaces() is used instead of wanX_ipaddr directly so tunnel-based WAN
 * protocols are covered as well.
 */
static void ctnf_ipv4_local_init(struct ctnf_ipv4_local *local)
{
	...
	char prefix[8];
	unsigned int bridge;
	int face, mwan_num, wan_unit;

	memset(local, 0, sizeof(*local));

	for (bridge = 0; bridge < BRIDGE_COUNT; ++bridge) {
		if ((inet_aton(bridge_nvram_get(bridge, "ipaddr", nv, sizeof(nv)), &ip) == 0) || (ip.s_addr == INADDR_ANY))
			continue;

		if ((inet_aton(bridge_nvram_get(bridge, "netmask", nv, sizeof(nv)), &mask) == 0) || (mask.s_addr == INADDR_ANY))
			continue;

		local->lan[local->lan_count].network = ip.s_addr & mask.s_addr;
		local->lan[local->lan_count].netmask = mask.s_addr;
		++local->lan_count;
	}
	...
	for (wan_unit = 1; wan_unit <= mwan_num; ++wan_unit) {
		...
		for (face = 0; face < wanfaces.count; ++face) {
			if (local->wan_count >= ASIZE(local->wan))
				break;

			if ((inet_aton(wanfaces.iface[face].ip, &ip) == 0) || (ip.s_addr == INADDR_ANY))
				continue;

			local->wan[local->wan_count++] = ip.s_addr;
		}
	}
}

/*
 * Return non-zero when addr is on the local side of an IPv4 connection.
 *
 * A LAN client is local when its address belongs to any configured bridge
 * subnet. Traffic generated by the router is also local when its source
 * exactly matches one of the router's WAN addresses. All other sources retain
 * the existing reply-direction/de-NAT handling.
 */
static int ctnf_ipv4_source_is_local(const struct ctnf_ipv4_local *local, const char *addr)
{
	struct in_addr ip;
	unsigned int i;
	...
	for (i = 0; i < local->lan_count; ++i) {
		if ((ip.s_addr & local->lan[i].netmask) == local->lan[i].network)
			return 1;
	}

	for (i = 0; i < local->wan_count; ++i) {
		if (ip.s_addr == local->wan[i])
			return 1;
	}
 ...
 */
	...
	if ((f = fopen("/proc/net/ip_conntrack", "r")) != NULL) {
		...
		while (fgets(s, sizeof(s), f)) {
			...
			switch (family) {
				case 2:
					if (!ctnf_ipv4_source_is_local(&local, src)) {
						dir_reply = 1;
						/* de-nat */
...
void asp_ctrate(int argc, char **argv)
{
	...
	while (fgets(sa, sizeof(sa), a)) {
		...
		switch(a_fam) {
			case 2:
				if (!ctnf_ipv4_source_is_local(&local, a_src))
					dir_reply = 1;
