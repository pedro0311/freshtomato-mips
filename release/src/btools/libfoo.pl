...
#	fixDynDep("libbcm.so", "libc.so.0");
	...
	fixDynDep("php-cgi", "libxml2.so.2.9.9");
	...
	fixDynDep("php-cli", "libxml2.so.2.9.9");
...
#shibby
genSO("${root}/usr/lib/libcurl.so.4.5.0", "${router}/libcurl/lib/.libs/libcurl.a", "", "-L${router}/zlib -L${router}/openssl");
...
genSO("${root}/usr/lib/libxml2.so.2.9.9", "${router}/libxml2/.libs/libxml2.a", "", "-L${router}/zlib");
