...
sub fixDyn
{
	...
	fixDynDep("minidlna", "libsqlite3.so.3.50.1");
...
# ipset modules
	...
	fixDynDep("php-cgi", "libsqlite3.so.3.50.1");
	...
	fixDynDep("php-cli", "libsqlite3.so.3.50.1");
	...
	fixDynDep("php-fpm", "libsqlite3.so.3.50.1");
...
genSO("${root}/usr/lib/libpng16.so.16.49.0", "${router}/libpng/staged/usr/lib/libpng16.a", "${stripshared}", "-L${router}/zlib");
if ($is_arm ne "y") { # MIPS
